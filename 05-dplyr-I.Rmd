...
Las columnas son: `id_estacion`, `temperatura_maxima`, `temperatura_minima` y `provincia`.
...
Si te fijás en la variable `paises`, ésta está intacta:
...
Para guardar la tabla con las tres columnas `anio`, `pais` y `pbi_per_capita` tenés que asignar el resultado a una nueva variable.
...
La mayoría de los análisis consisten en varios pasos (en el primer desafío usaste 4 pasos para *Cambiar el ejemplo: calcular las temperaturas medias de una serie de estaciones*).
...
Cómo vimos, el primero argumento de todas las funciones de dplyr espera un data.frame y justamente el operador `%>%` toma el data.frame `paises` y se lo pasa al primer argumento de `select()`.
Luego el data.frame resultante de seleccionar las columnas anio, país y pib_per_capita *pasa* como el primer argumento de la función `filter()` gracias al `%>%`.
