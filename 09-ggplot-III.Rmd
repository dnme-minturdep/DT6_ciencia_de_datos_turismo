...
En [el capítulo de ggplot2](gráficos-con-ggplot2.html) vimos el funcionamiento básico de ggplot2.
...
Cuando ggplot2 *mapea* distintos colores a los distintos valores de la columna `region`, lo que define qué color le corresponde a qué color de `región` es una **escala**.
...
Una distinción importante es entre las escalas continuas y discretas.
...
En el caso de nuestro gráfico, donde los colores representan distintas regiones del país, se trata de una variable cateórica, por la que neceistamos escalas discretas.

### Escalas de colores

La escala de colores que usa ggplot2 por defecto no es de las mejores, de hecho las personas que tienen daltonísmo muy posiblemente no logren diferenciar todas las líneas.
Una escala o paleta de colores continua muy usada es [**viridis**](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html) que fue creada justamente para resolver este y otros problemas.
Para valores discretos existe una gran familia de paletas de colores llamada [**ColorBrewer**](https://colorbrewer2.org/).
...
A esta altura, estamos ante un momento importante en la creación de un gráfico.
...
Del código anterior surge algo muy importante: es posible generar capas en un gráfico usando una tabla *distinta* al que usamos para graficar las capas anteriores.
...
En este código, `anotate()` dibuja un rectángulo (el equivalente a `geom_rect()`) donde el límite izquierdo es el primero de enero de 2020, y el derecho es el primero de abril de 2021.
...
Entonces vamos a modificar el eje y agregando una capa con `scale_y_continuous()` (porque `total` es una variable continua) y usando el argumento `labels`.
...
Para cambiar el nombre de los pabeles hay que primero generar un vector que asigne cada nombre "de computadora" (o sea, sin espacios ni mayúsculas ni tildes) al nombre "para personas". 
La forma de hacer esto es creando un vector con nombres. 

```{r}
etiquetas_regiones <- c("Buenos Aires", "Córdoba", "Cuyo", "Litoral", "Norte", "Patagonia")
names(etiquetas_regiones) <- c("buenos-aires", "cordoba", "cuyo", "litoral", "norte", "patagonia")
...
Un último elemento que conviene agrear a todo gráfico que va a circular en público es el contexto. 
Algún texto que describa el gráfico y la fuente de datos. 
Esto se puede lograr agregando título, subtítulo y caption dentro de la función `labs()`.
