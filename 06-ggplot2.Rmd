...
Este mapeo se hace **siempre** con la función `aes()` (que viene de *aesthetics*). 
...
Pero, como se ve, el esto sólo genera el área del gráfico y los ejes. 
...
La escala de colores es lo que define, por ejemplo, que los puntos donde la variable `region` toma el valor `"patagonia"` van a tener el color rosa ([●]{style="color:#FB61D7"}), donde toma el valor `córdoba"`, mostaza ([●]{style="color:#B79F00"}), etc...
...
Y este resultado no es ni más ni menos que el data frame que neceistás para hacer el gráfico.
...
Para esta sección vamos a usar datos de la [Encuesta de Viajes y Turismo de los Hogares](http://datos.yvera.gob.ar/dataset/encuesta-viajes-turismo-hogares-evyth) (EVYTH). 
...
ggplot(gastos, aes(factor(quintil), gasto)) +
  geom_boxplot() + 
  scale_y_log10() +
  facet_wrap(~region_destino) 
```
