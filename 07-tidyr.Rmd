...
A esta altura del libro vimos varios set de datos, cada uno tiene una pinta un poco distinta y trabajar con ellos presenta distintos desafíos. 
...
Cuando hablamos de datos en formato "largo" o "tidy", nos referimos a aquellos set de datos en los cuales:

-   cada fila es una observación
-   cada columna es una variable

Los datos en formato "ancho" pueden ser muy variados y son un poco más complejo de definir pero la idea general es que:
...
Si no lo tenés instalado, podés hacer con el comando:

```{r eval = FALSE}
install.packages("tidyr")
```

Cómo siempre, recordá que esto solo se hace una vez y es recomendable hacerlo desde la consola para que no quede en un chunk de código por accidente.
...
Por ejemplo, es complicado hacer series de tiempo de los visitantes de uan región porque la información está distribuida entre varias columnas. Tampoco sería simple hacer cuentas por regiones o por tipo de residente con este formato de tabla.
...
Po ejemplo `starts_with()` que, como su nombre en inglés lo indica, selecciona las columnas que *empiezan con* una determinada cadena de caracteres.
en este caso le decimos que queremos todas las columnas *menos* las que están mencionadas. Por eso hay un "-" antes del vector de columnas. Entonces, el vector `-c("indice_tiempo", "residentes", "no_residentes", "total")` le dice a `pivot_longer()` que seleccione todas las columnas menos las que están mencionadas.
...
La columna `region_visitante` todavía no es muy útil porque contiene 2 datos, la región (Buenos Aires, Cuyo, Litoral, etc.) y el tipo de visitante y la suma de ambos.
Sería mejor separar esta información en dos columnas llamadas "region" y "resinte".
...
Si hubieamos usado la función `separate()` directamente hubieramos terminado con 3 columnas una con el datos "buenos", otra con el datos "aires" y la última como el dato "residentes". Ni hablar si pensamos en "norte_residentes", nos quedaríamos con 2 columnas. 
Por esa razón primero tuvimos que manipular las columnas con `mutate()` para reemplazar "buenos_aires" por "buenos-aires" y "no_residentes" por "noresidentes". Si el plan es poder usar el "_" como separador entre la región y el tipo de visitante. 
...
Pero con los datos así no podrías hacer un gráfico de puntos que muestre la relación entre el residentes y no residentes de vida como en la [sección de gráficos](gráficos-con-ggplot2.html#segunda-capa-geometrías).
...
Ahora vamos a seguir trabajando con las base de datos de `parques_largo` y de paso unirlo a una nueva base de datos `hoeles` que contiene información de viajeron por región de destino y origen a lo largo del tiempo.

Para que los datos sean más manejable y poder ver que es lo que sucede vamos a quedarnos solo con la información de 2018.
...
::: {.alert .alert-success}
En el código de más arriba volvimos a usar la librería lubridate. En este caso la función `year()` permite extraer el parte del año de la fecha que está guardada en la columna `indice_tiempo`. 
...
En este caso, perdemos las filas de `hoteles_2018` que no encontraron coincidencia en `parques_2018`, es decir la región de CABA. La tabla resultante es igual a la tabla que generamos en el ejemplo anterior con 216 fialas.
